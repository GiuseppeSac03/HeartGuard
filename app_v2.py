...
# Variabili che vogliamo salvare
...
def go_next_step():
    st.session_state.step += 1
    # Nessun experimental_rerun, ci affidiamo al “rerun” di default
...
# Definizione delle feature numeriche e di quelle totali
...
if st.session_state.step == 1:
    ...
    with col2:
        st.write("")  # placeholder per allineamento
...
# STEP 6: Abitudini di Vita (Fumo, Alcol, Attività Fisica)
...
# STEP 7: Riepilogo e Calcolo Previsione
elif st.session_state.step == 7:
    st.subheader("Step 7: Riepilogo e Previsione")

    # Calcoliamo BMI e rapporto pressione
    ...
    # Bottoni Indietro / Calcola Previsione
    col1, col2 = st.columns([1,1])
    with col1:
        st.button("Indietro", on_click=go_prev_step)

    # Calcolo della previsione solo se il modello e lo scaler sono disponibili
    if model is not None and scaler is not None:
        if st.session_state.ap_hi > st.session_state.ap_lo:
            if col2.button("Calcola Previsione", type="primary"):
                # Preparazione dati input
    ...
    # Mostriamo il risultato se la predizione è stata fatta
    if st.session_state.prediction_done:
        ...
        # Spiegazione LIME (con loading spinner)
        if explainer is not None:
            try:
                ...
                # Ordiniamo le feature per importanza e generiamo suggerimenti
